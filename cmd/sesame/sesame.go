...
import (
	...
	"github.com/projectsesame/sesame/internal/build"
	"github.com/projectsesame/sesame/internal/envoy"
...
func main() {
	...
	app := kingpin.New("sesame", "Sesame Kubernetes ingress controller.")
	...
	cli := app.Command("cli", "A CLI client for the Sesame Kubernetes ingress controller.")
	var client Client
	cli.Flag("sesame", "Sesame host:port.").Default("127.0.0.1:8001").StringVar(&client.SesameAddr)
	cli.Flag("cafile", "CA bundle file for connecting to a TLS-secured Sesame.").Envar("CLI_CAFILE").StringVar(&client.CAFile)
	cli.Flag("cert-file", "Client certificate file for connecting to a TLS-secured Sesame.").Envar("CLI_CERT_FILE").StringVar(&client.ClientCert)
	cli.Flag("key-file", "Client key file for connecting to a TLS-secured Sesame.").Envar("CLI_KEY_FILE").StringVar(&client.ClientKey)
	...
	version := app.Command("version", "Build information for Sesame.")
	...
	case serve.FullCommand():
		...
		if err != nil {
			log.WithError(err).Fatal("unable to initialize Server dependencies required to start Sesame")
		}

		if err := serve.doServe(); err != nil {
			log.WithError(err).Fatal("Sesame server failed")
		}
