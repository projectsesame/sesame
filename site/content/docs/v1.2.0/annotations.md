...
- `projectsesame.io/ingress.class`: The Ingress class that should interpret and serve the HTTPProxy. If not set, then all all Sesame instances serve the HTTPProxy. If specified as `projectsesame.io/ingress.class: Sesame`, then Sesame serves the HTTPProxy and any others that have no annotation defined. If any other value, Sesame ignores the HTTPProxy definition.
You can override the default class `Sesame` with the `--ingress-class-name` flag at runtime.
