...
## Q: What's the relationship between Sesame and Istio? 
...
Sesame focuses on north-south traffic only -- on making Envoy available to Kubernetes users as a simple, reliable load balancing solution.
...
As Sesame continues to develop -- who knows what the future holds?

## Q: What are the differences between Ingress and HTTPProxy?

Sesame supports both the Kubernetes Ingress API and the HTTPProxy API, a custom resource that enables a richer and more robust experience for configuring ingress into your Kubernetes cluster.
...
Sesame programs Envoy with a tight binding between TLS server names and HTTP virtual host routing tables.
...
HTTP/2 support can be disabled by setting the `default-http-versions` field in the Sesame [configuration file][3].

## Q: Why is the Envoy container not accepting connections even though Sesame is running?

Sesame does not configure Envoy to listen on a port unless there is traffic to be served.
For example, if you have not configured any TLS Ingress objects then Sesame does not command Envoy to open the secure listener (port 443 in the example deployment).
