...
spec:
    virtualhost:
        ...
        routes:
        - match: /sesame
          # the timeout applied to the requests to "/sesame" only, independent of other routes.
          timeoutPolicy:
              # the request timeout applied on this route, it is the maximum time that a client will
              # await a response to its request.
              request: 1s
          # the retryPolicy described here are local to match and apply for "/sesame" only
          retryPolicy:
        ...
        - match: /static-site
          ...
          timeoutPolicy:
              request: 200ms
              idle: 1s
          # retry for this route is independent of "/sesame" retryPolicy.
          retryPolicy:
          	count: 2
```
...
```go
// sesame/apis/sesame/v1beta1/ingressroute.go
...
```go
// Approach: sesame/internal/dag/dag.go
