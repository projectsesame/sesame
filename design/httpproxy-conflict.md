...
To fix this, Sesame will treat errors and warning differently and where possible, make a best effort to serve partially valid configurations.
...
- User: A request to an application, routing through Envoy looking to access whatever resource an application running in Kubernetes should serve. This user is not aware of how the application is deployed or that it uses Kubernetes or Sesame.
...
Sesame has further extended this to add `Conditions` which allows for a set of `Errors` and `Warnings` which inform users of errors in the HTTPProxy resource.

It's difficult to find errors and block them from breaking valid configurations since Sesame currently processes objects after they are committed to the API server.
Due to this, Sesame cannot block changes to resources before they are committed like an `Admission Controller` would be able to do (See alternatives: `HTTPProxyInstance`).

Sesame currently invalidates the entire resource if a configuration error is encountered which can cause downtime for specific routes or even the entire virtual host.

When processing HTTPProxy objects, some problems should not stop the rest of the object from being processed.
Sesame will set an object to be status `Error` if the request response to the `User` is changed from what is configured.
Sesame will set a `Warning` when the object has an issue, but the response a `User` sees when accessing the application for which the resource is configured is not modified.
...
## Non Goals
- Change how Sesame processes objects today from informers set against the Kubernetes API
...
- Sesame stops processing the rest of the spec
- Sesame does not program any part of the HTTPProxy in Envoy
...
- Sesame continues processing the rest of the spec
- Sesame does program the unaffected (valid) part of the HTTPProxy in Envoy
...
- Sesame continues processing the rest of the spec
- Sesame does program the unaffected part of the HTTPProxy in Envoy
...
## High-Level Design
Sesame will set the error or warning condition when a problem is encountered, but do its best to still serve valid configurations.
...
### Invalid Include
Sesame utilizes an inclusion model to allow for proper route/header delegation across namespaces and teams within a Kubernetes cluster.
If an include does not have a matching HTTPProxy.Name or HTTPProxy.Namespace, then Sesame will set an error in the Conditions of the object.
...
### Invalid Route
If a route contains configuration errors or warnings, the following table outlines how Sesame will react and what response will be sent.  
...
### Invalid Service
If a service contains configuration errors or warnings, the following table outlines how Sesame will react and what response will be sent.

#### Single Service
For the case where only a single service is referenced from a `Spec.Route` of an HTTPProxy and has an `Error`, Sesame will return a `503` status code and also set an error in the Conditions of the object.

#### Multiple Services
For the case where multiple services are referenced from a `Spec.Route` of an HTTPProxy and has an `Error`, Sesame will set the invalid service weight to be `zero` so that no traffic is routed to the service in error and will also set an error in the Conditions of the object.
...
This could also be later layered onto Sesame without changing any logic defined in this proposal. 
