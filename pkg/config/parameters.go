...
func (s ServerType) Validate() error {
	switch s {
	case SesameServerType, EnvoyServerType:
		return nil
	default:
...
// Used for Sesame configuration YAML file parsing, otherwise we could use K8s types.NamespacedName.
...
func (p *MetricsParameters) Validate() error {
	if err := p.Sesame.Validate(); err != nil {
		return fmt.Errorf("metrics.sesame: %v", err)
	}
...
func Defaults() Parameters {
	...
	return Parameters{
		...
		Server: ServerParameters{
			XDSServerType: SesameServerType,
		},
