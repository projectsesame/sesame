...
import (
	...
	"github.com/projectsesame/sesame/internal/k8s/mocks"
...
func TestServiceStatusLoadBalancerWatcherOnAdd(t *testing.T) {
	...
	svc.Status.LoadBalancer.Ingress = []v1.LoadBalancerIngress{{Hostname: "projectsesame.io"}}
	sw.OnAdd(&svc)
	got, ok := recv()
	if !ok {
	...
	want := v1.LoadBalancerStatus{
		Ingress: []v1.LoadBalancerIngress{{Hostname: "projectsesame.io"}},
	}
...
func TestStatusAddressUpdater(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"ingress: not-configured ingressclass, spec field set to default, should update": {
			status:           ipLBStatus,
			ingressClassName: "",
			preop:            simpleIngressGenerator(objName, "", "sesame", emptyLBStatus),
			postop:           simpleIngressGenerator(objName, "", "sesame", ipLBStatus),
		},
...
func TestStatusAddressUpdater_Gateway(t *testing.T) {
	...
	hostnameLBStatus := v1.LoadBalancerStatus{
		Ingress: []v1.LoadBalancerIngress{
			{
				Hostname: "ingress.projectsesame.io",
			},
		},
	}
	...
	testCases := map[string]struct {
		...
		"happy path (IP)": {
			status:                     ipLBStatus,
			gatewayClassControllerName: "projectsesame.io/sesame",
			preop: &gatewayapi_v1alpha2.Gateway{
				...
				Spec: gatewayapi_v1alpha2.GatewaySpec{
					GatewayClassName: gatewayapi_v1alpha2.ObjectName("sesame-gatewayclass"),
				},
	...
	for name, tc := range testCases {
		t.Run(name+" OnAdd", func(t *testing.T) {
			...
			isu := StatusAddressUpdater{
				Logger:                log,
				GatewayControllerName: "projectsesame.io/sesame",
				Cache:                 mockCache,
				LBStatus:              tc.status,
				StatusUpdater:         &suc,
			}
...
func simpleProxyGenerator(name, ingressClass string, lbstatus v1.LoadBalancerStatus) *sesame_api_v1.HTTPProxy {
