...
func TestJSONFileAccessLog(t *testing.T) {
	tests := map[string]struct {
		path    string
		headers sesame_api_v1alpha1.AccessLogFields
		want    []*envoy_accesslog_v3.AccessLog
	}{
		"only timestamp": {
			path:    "/dev/stdout",
			headers: sesame_api_v1alpha1.AccessLogFields([]string{"@timestamp"}),
			want: []*envoy_accesslog_v3.AccessLog{{
				Name: wellknown.FileAccessLog,
				ConfigType: &envoy_accesslog_v3.AccessLog_TypedConfig{
					TypedConfig: protobuf.MustMarshalAny(&envoy_file_v3.FileAccessLog{
		...
		"custom fields should appear": {
			path: "/dev/stdout",
			headers: sesame_api_v1alpha1.AccessLogFields([]string{
				"@timestamp",
				"method",
