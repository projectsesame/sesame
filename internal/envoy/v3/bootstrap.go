...
func WriteBootstrap(c *envoy.BootstrapConfig) error {
	...
	if c.ResourcesDir != "" {
		...
		// Refer Issue: https://github.com/projectsesame/sesame/issues/3264
...
func bootstrap(c *envoy.BootstrapConfig) ([]bootstrapf, error) {
	...
	// - SDS resource for xDS client certificate and key for authenticating Envoy towards Sesame.
	// - SDS resource for trusted CA certificate for validating Sesame server certificate.
...
func bootstrapConfig(c *envoy.BootstrapConfig) *envoy_bootstrap_v3.Bootstrap {
	return &envoy_bootstrap_v3.Bootstrap{
		DynamicResources: &envoy_bootstrap_v3.Bootstrap_DynamicResources{
			LdsConfig: ConfigSource("sesame"),
			CdsConfig: ConfigSource("sesame"),
		},
		StaticResources: &envoy_bootstrap_v3.Bootstrap_StaticResources{
			Clusters: []*envoy_cluster_v3.Cluster{{
				...
				Name:                 "sesame",
				AltStatName:          strings.Join([]string{c.Namespace, "sesame", strconv.Itoa(c.GetXdsGRPCPort())}, "_"),
