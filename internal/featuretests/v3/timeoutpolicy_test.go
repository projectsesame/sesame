...
func TestTimeoutPolicyRequestTimeout(t *testing.T) {
	...
	i1 := &networking_v1.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				"projectsesame.io/response-timeout": "1m20s",
			},
		},
	...
	i2 := &networking_v1.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: svc.Namespace,
			Annotations: map[string]string{
				"projectsesame.io/response-timeout": "infinity",
			},
		},
		Spec: i1.Spec,
	}
	...
	i3 := &networking_v1.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: svc.Namespace,
			Annotations: map[string]string{
				"projectsesame.io/response-timeout": "monday",
			},
		},
		Spec: i2.Spec,
	}
	...
	i4 := &networking_v1.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: svc.Namespace,
			Annotations: map[string]string{
				"projectsesame.io/request-timeout":  "90s",
				"projectsesame.io/response-timeout": "99s",
			},
		},
		Spec: i2.Spec,
	}
	...
	// assert that projectsesame.io/response-timeout takes priority.
	c.Request(routeType).Equals(&envoy_discovery_v3.DiscoveryResponse{
		Resources: resources(t,
			envoy_v3.RouteConfiguration("ingress_http",
				envoy_v3.VirtualHost("*",
