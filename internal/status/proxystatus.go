...
func (pu *ProxyUpdate) ConditionFor(cond ConditionType) *sesame_api_v1.DetailedCondition {
	...
	if !ok {
		...
		if cond == ValidCondition {
			newDc.Status = sesame_api_v1.ConditionTrue
			newDc.Reason = "Valid"
			newDc.Message = "Valid HTTPProxy"
		} else {
			newDc.Status = sesame_api_v1.ConditionFalse
		}
...
func (pu *ProxyUpdate) Mutate(obj client.Object) client.Object {
	o, ok := obj.(*sesame_api_v1.HTTPProxy)
	if !ok {
	...
	validCond := proxy.Status.GetConditionFor(sesame_api_v1.ValidConditionType)

	switch validCond.Status {
	case sesame_api_v1.ConditionTrue:
	...
	case sesame_api_v1.ConditionFalse:
		if orphanCond, ok := validCond.GetError(sesame_api_v1.ConditionTypeOrphanedError); ok {
