...
type CacheEntry interface {
	...
	ConditionFor(ConditionType) *sesame_api_v1.DetailedCondition
}
...
func (c *Cache) GetStatusUpdates() []k8s.StatusUpdate {
	...
	for fullname, pu := range c.proxyUpdates {
		update := k8s.StatusUpdate{
			...
			Resource:       &sesame_api_v1.HTTPProxy{},
			Mutator:        pu,
		}
...
// from the cache, used by various things (`internal/sesame/metrics.go` and `internal/dag/status_test.go`)
...
// the cache internals.
func (c *Cache) ProxyAccessor(proxy *sesame_api_v1.HTTPProxy) (*ProxyUpdate, func()) {
	pu := &ProxyUpdate{
	...
	return pu, func() {
		...
		if ok {
			...
			if c.proxyUpdates[pu.Fullname].Conditions[ValidCondition].Status == sesame_api_v1.ConditionFalse {
				if pu.Conditions[ValidCondition].Status == sesame_api_v1.ConditionTrue {
					return
				}
			}
		}
