...
func isValidSecret(secret *v1.Secret) (bool, error) {
	...
	case v1.SecretTypeOpaque, "":
		...
		// will be Opaque Secrets that Sesame doesn't care about.
		...
		// length, Sesame can't use it, so return an error.
	...
	// (see https://github.com/projectsesame/sesame/issues/1644).
	if data := secret.Data[CACertificateKey]; len(data) > 0 {
