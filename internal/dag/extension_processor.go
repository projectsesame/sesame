...
func (p *ExtensionServiceProcessor) Run(dag *DAG, cache *KubernetesCache) {
	for _, e := range cache.extensions {
		...
		if len(validCondition.Errors) == 0 {
			validCondition.Status = sesame_api_v1.ConditionTrue
...
func (p *ExtensionServiceProcessor) buildExtensionService(
	...
	ext *sesame_api_v1alpha1.ExtensionService,
	validCondition *sesame_api_v1.DetailedCondition,
) *ExtensionCluster {
	tp, err := timeoutPolicy(ext.Spec.TimeoutPolicy)
	if err != nil {
		validCondition.AddErrorf(sesame_api_v1.ConditionTypeSpecError, "TimeoutPolicyNotValid",
	...
	if p.ClientCertificate != nil {
		...
		if err != nil {
			validCondition.AddErrorf(sesame_api_v1.ConditionTypeTLSError, "SecretNotValid",
	...
	case LoadBalancerPolicyCookie, LoadBalancerPolicyRequestHash:
		validCondition.AddWarningf(sesame_api_v1.ConditionTypeSpecError, "IgnoredField",
	...
	if v := ext.Spec.UpstreamValidation; v != nil {
		...
		if !cache.DelegationPermitted(caCertNamespacedName, ext.Namespace) {
			validCondition.AddErrorf(sesame_api_v1.ConditionTypeTLSError, "CACertificateNotDelegated",
		...
		if uv, err := cache.LookupUpstreamValidation(v, caCertNamespacedName); err != nil {
			validCondition.AddErrorf(sesame_api_v1.ConditionTypeSpecError, "TLSUpstreamValidation",
			...
			// future.
			//
			// TODO(jpeach): expose SNI in the API, https://github.com/projectsesame/sesame/issues/2893.
			extension.SNI = uv.SubjectName
		}

		if extension.Protocol != "h2" {
			validCondition.AddErrorf(sesame_api_v1.ConditionTypeSpecError, "InconsistentProtocol",
	...
	for _, target := range ext.Spec.Services {
		...
		// subvert the Sesame installation.
		...
		if err != nil {
			validCondition.AddErrorf(sesame_api_v1.ConditionTypeServiceError, "ServiceUnresolvedReference",
				"unresolved service %q: %s", svcName, err)
			continue
		}

		// TODO(jpeach): Add ExternalName support in https://github.com/projectsesame/sesame/issues/2875.
		// TODO(youngnick): If ExternalName support is added, we must pass down the EnableExternalNameService bool
		// and check it first.
		if svc.Spec.ExternalName != "" {
			validCondition.AddErrorf(sesame_api_v1.ConditionTypeServiceError, "UnsupportedServiceType",
