		...
		"explicit 0s timeout": {
			i: &networking_v1.Ingress{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						"projectsesame.io/retry-on":        "5xx",
						"projectsesame.io/per-try-timeout": "0s",
					},
				},
			},
			want: &RetryPolicy{
...
func TestRetryPolicy(t *testing.T) {
	tests := map[string]struct {
		...
		"retry on": {
			rp: &sesame_api_v1.RetryPolicy{
				RetryOn: []sesame_api_v1.RetryOn{"gateway-error", "connect-failure"},
			},
			want: &RetryPolicy{
...
func TestTimeoutPolicy(t *testing.T) {
	tests := map[string]struct {
		tp      *sesame_api_v1.TimeoutPolicy
		want    TimeoutPolicy
		wantErr bool
	}{
		"nil timeout policy": {
...
func TestLoadBalancerPolicy(t *testing.T) {
	tests := map[string]struct {
		lbp  *sesame_api_v1.LoadBalancerPolicy
		want string
	}{
		"nil": {
			lbp:  nil,
...
func TestRateLimitPolicy(t *testing.T) {
	tests := map[string]struct {
		in      *sesame_api_v1.RateLimitPolicy
					...
					Descriptors: []sesame_api_v1.RateLimitDescriptor{
						{
							Entries: []sesame_api_v1.RateLimitDescriptorEntry{
								{
									GenericKey: &sesame_api_v1.GenericKeyDescriptor{
										Key:   "generic-key-key",
								...
								{
									RequestHeader: &sesame_api_v1.RequestHeaderDescriptor{
										HeaderName:    "X-Header",
					...
					Descriptors: []sesame_api_v1.RateLimitDescriptor{
						{
							Entries: []sesame_api_v1.RateLimitDescriptorEntry{
								{
									RequestHeaderValueMatch: &sesame_api_v1.RequestHeaderValueMatchDescriptor{
										Headers: []sesame_api_v1.HeaderMatchCondition{
											{
												Name:       "X-Header",
												NotPresent: true,
											},
