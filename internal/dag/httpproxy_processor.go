...
func defaultExtensionRef(ref sesame_api_v1.ExtensionServiceReference) sesame_api_v1.ExtensionServiceReference {
	if ref.APIVersion == "" {
		ref.APIVersion = sesame_api_v1alpha1.GroupVersion.String()
...
type HTTPProxyProcessor struct {
	...
	// See https://github.com/projectsesame/sesame/security/advisories/GHSA-5ph6-qq5x-7jwc for details.
...
func (p *HTTPProxyProcessor) Run(dag *DAG, source *KubernetesCache) {
	...
	for meta := range p.orphaned {
		...
		if ok {
			...
			pa.ConditionFor(status.ValidCondition).AddError(sesame_api_v1.ConditionTypeOrphanedError,
				"Orphaned",
...
func (p *HTTPProxyProcessor) computeHTTPProxy(proxy *sesame_api_v1.HTTPProxy) {
	pa, commit := p.dag.StatusCache.ProxyAccessor(proxy)
	...
	if isBlank(host) {
		validCond.AddError(sesame_api_v1.ConditionTypeVirtualHostError, "FQDNNotSpecified",
	...
	if !p.rootAllowed(proxy.Namespace) {
		validCond.AddError(sesame_api_v1.ConditionTypeRootNamespaceError, "RootProxyNotAllowedInNamespace",
	...
	if len(proxy.Spec.Routes) == 0 && len(proxy.Spec.Includes) == 0 && proxy.Spec.TCPProxy == nil {
		validCond.AddError(sesame_api_v1.ConditionTypeSpecError, "NothingDefined",
	...
	if tls := proxy.Spec.VirtualHost.TLS; tls != nil {
		if tls.Passthrough && tls.EnableFallbackCertificate {
			validCond.AddError(sesame_api_v1.ConditionTypeTLSError, "TLSIncompatibleFeatures",
		...
		if !isBlank(tls.SecretName) && tls.Passthrough {
			validCond.AddError(sesame_api_v1.ConditionTypeTLSError, "TLSConfigNotValid",
		...
		if !tls.Passthrough {
			...
			if err != nil {
				validCond.AddErrorf(sesame_api_v1.ConditionTypeTLSError, "SecretNotValid",
					"Spec.VirtualHost.TLS Secret %q is invalid: %s", tls.SecretName, err)
				return
			}

			if !p.source.DelegationPermitted(secretName, proxy.Namespace) {
				validCond.AddErrorf(sesame_api_v1.ConditionTypeTLSError, "DelegationNotPermitted",
			...
			if tls.EnableFallbackCertificate {
				...
				if err != nil {
					validCond.AddErrorf(sesame_api_v1.ConditionTypeTLSError, "FallbackNotValid",
				...
				if !p.source.DelegationPermitted(*p.FallbackCertificate, proxy.Namespace) {
					validCond.AddErrorf(sesame_api_v1.ConditionTypeTLSError, "FallbackNotDelegated",
			...
			if tls.ClientValidation != nil {
				...
				if tls.ClientValidation.CACertificate != "" {
					...
					if err != nil {
						...
						validCond.AddErrorf(sesame_api_v1.ConditionTypeTLSError, "ClientValidationInvalid",
			...
			if proxy.Spec.VirtualHost.AuthorizationConfigured() {
				...
				if ref.APIVersion != sesame_api_v1alpha1.GroupVersion.String() {
					validCond.AddErrorf(sesame_api_v1.ConditionTypeAuthError, "AuthBadResourceVersion",
				...
				if ext == nil {
					validCond.AddErrorf(sesame_api_v1.ConditionTypeAuthError, "ExtensionServiceNotFound",
				...
				if err != nil {
					validCond.AddErrorf(sesame_api_v1.ConditionTypeAuthError, "AuthResponseTimeoutInvalid",
	...
	if proxy.Spec.TCPProxy != nil {
		if !tlsEnabled {
			validCond.AddError(sesame_api_v1.ConditionTypeTCPProxyError, "TLSMustBeConfigured",
	...
	if err != nil {
		validCond.AddErrorf(sesame_api_v1.ConditionTypeCORSError, "PolicyDidNotParse",
	...
	if err != nil {
		validCond.AddErrorf(sesame_api_v1.ConditionTypeRouteError, "RateLimitPolicyNotValid",
...
func (p *HTTPProxyProcessor) computeRoutes(
	validCond *sesame_api_v1.DetailedCondition,
	rootProxy *sesame_api_v1.HTTPProxy,
	proxy *sesame_api_v1.HTTPProxy,
	conditions []sesame_api_v1.MatchCondition,
	visited []*sesame_api_v1.HTTPProxy,
	enforceTLS bool,
) []*Route {
	for _, v := range visited {
		...
		if v.Name == proxy.Name && v.Namespace == proxy.Namespace {
			...
			validCond.AddErrorf(sesame_api_v1.ConditionTypeIncludeError, "IncludeCreatesCycle",
	...
	if includeMatchConditionsIdentical(proxy.Spec.Includes) {
		validCond.AddError(sesame_api_v1.ConditionTypeIncludeError, "DuplicateMatchConditions",
	...
	for _, include := range proxy.Spec.Includes {
		...
		if err := pathMatchConditionsValid(include.Conditions); err != nil {
			validCond.AddErrorf(sesame_api_v1.ConditionTypeIncludeError, "PathMatchConditionsNotValid",
				"include: %s", err)
			continue
		}

		if err := headerMatchConditionsValid(include.Conditions); err != nil {
			validCond.AddError(sesame_api_v1.ConditionTypeRouteError, "HeaderMatchConditionsNotValid",
				err.Error())
			continue
		}
		...
		if !ok {
			validCond.AddErrorf(sesame_api_v1.ConditionTypeIncludeError, "IncludeNotFound",
		...
		if includedProxy.Spec.VirtualHost != nil {
			validCond.AddErrorf(sesame_api_v1.ConditionTypeIncludeError, "RootIncludesRoot",
	...
	for _, route := range proxy.Spec.Routes {
		if err := pathMatchConditionsValid(route.Conditions); err != nil {
			validCond.AddErrorf(sesame_api_v1.ConditionTypeRouteError, "PathMatchConditionsNotValid",
				"route: %s", err)
			return nil
		}
		...
		if err != nil {
			validCond.AddErrorf(sesame_api_v1.ConditionTypeRouteError, "RequestHeadersPolicyInvalid",
		...
		if err != nil {
			validCond.AddErrorf(sesame_api_v1.ConditionTypeRouteError, "ResponseHeaderPolicyInvalid",
		...
		if err != nil {
			validCond.AddErrorf(sesame_api_v1.ConditionTypeRouteError, "CookieRewritePoliciesInvalid",
		...
		if len(route.Services) < 1 && route.RequestRedirectPolicy == nil {
			validCond.AddError(sesame_api_v1.ConditionTypeRouteError, "NoServicesPresent",
		...
		if err != nil {
			validCond.AddErrorf(sesame_api_v1.ConditionTypeRouteError, "TimeoutPolicyNotValid",
		...
		if len(route.GetPrefixReplacements()) > 0 {
			if !r.HasPathPrefix() {
				validCond.AddError(sesame_api_v1.ConditionTypePrefixReplaceError, "MustHavePrefix",
			...
			if reason, err := prefixReplacementsAreValid(route.GetPrefixReplacements()); err != nil {
				validCond.AddError(sesame_api_v1.ConditionTypePrefixReplaceError, reason, err.Error())
				return nil
			}
		...
		for _, service := range route.Services {
			if service.Port < 1 || service.Port > 65535 {
				validCond.AddErrorf(sesame_api_v1.ConditionTypeServiceError, "ServicePortInvalid",
			...
			if err != nil {
				validCond.AddErrorf(sesame_api_v1.ConditionTypeServiceError, "ServiceUnresolvedReference",
			...
			if err != nil {
				validCond.AddError(sesame_api_v1.ConditionTypeServiceError, "UnsupportedProtocol", err.Error())
				return nil
			}
			...
			if (protocol == "tls" || protocol == "h2") && service.UpstreamValidation != nil {
				...
				if !p.source.DelegationPermitted(caCertNamespacedName, proxy.Namespace) {
					validCond.AddErrorf(sesame_api_v1.ConditionTypeTLSError, "CACertificateNotDelegated",
				...
				if err != nil {
					validCond.AddErrorf(sesame_api_v1.ConditionTypeServiceError, "TLSUpstreamValidation",
			...
			if err != nil {
				validCond.AddErrorf(sesame_api_v1.ConditionTypeServiceError, "RequestHeadersPolicyInvalid",
					"%s on request headers", err)
				return nil
			}
			respHP, err := headersPolicyService(p.ResponseHeadersPolicy, service.ResponseHeadersPolicy, dynamicHeaders)
			if err != nil {
				validCond.AddErrorf(sesame_api_v1.ConditionTypeServiceError, "ResponseHeadersPolicyInvalid",
					"%s on response headers", err)
				return nil
			}
			...
			if service.Mirror && r.MirrorPolicy != nil {
				validCond.AddError(sesame_api_v1.ConditionTypeServiceError, "OnlyOneMirror",
...
func (p *HTTPProxyProcessor) processHTTPProxyTCPProxy(validCond *sesame_api_v1.DetailedCondition, httpproxy *sesame_api_v1.HTTPProxy, visited []*sesame_api_v1.HTTPProxy, host string) bool {
	...
	if len(tcpproxy.Services) > 0 && tcpProxyInclude != nil {
		validCond.AddError(sesame_api_v1.ConditionTypeTCPProxyError, "NoServicesAndInclude",
	...
	case LoadBalancerPolicyCookie, LoadBalancerPolicyRequestHash:
		validCond.AddWarningf(sesame_api_v1.ConditionTypeTCPProxyError, "IgnoredField",
	...
	if len(tcpproxy.Services) > 0 {
		...
		for _, service := range httpproxy.Spec.TCPProxy.Services {
			...
			s, err := p.dag.EnsureService(m, intstr.FromInt(service.Port), p.source, p.EnableExternalNameService)
			if err != nil {
				validCond.AddErrorf(sesame_api_v1.ConditionTypeTCPProxyError, "ServiceUnresolvedReference",
	...
	if tcpProxyInclude == nil {
		...
		validCond.AddError(sesame_api_v1.ConditionTypeTCPProxyError, "NothingDefined",
	...
	if !ok {
		validCond.AddErrorf(sesame_api_v1.ConditionTypeTCPProxyIncludeError, "IncludeNotFound",
	...
	if dest.Spec.VirtualHost != nil {
		...
		validCond.AddErrorf(sesame_api_v1.ConditionTypeTCPProxyIncludeError, "RootIncludesRoot",
			"root httpproxy cannot include another root httpproxy")
		return false
	}
	...
	for _, hp := range visited {
		if dest.Name == hp.Name && dest.Namespace == hp.Namespace {
			...
			validCond.AddErrorf(sesame_api_v1.ConditionTypeTCPProxyIncludeError, "IncludeCreatesCycle",
...
// validHTTPProxies returns a slice of *sesame_api_v1.HTTPProxy objects.
// invalid HTTPProxy objects are excluded from the slice and their status
// updated accordingly.
func (p *HTTPProxyProcessor) validHTTPProxies() []*sesame_api_v1.HTTPProxy {
	// ensure that a given fqdn is only referenced in a single HTTPProxy resource
	var valid []*sesame_api_v1.HTTPProxy
	fqdnHTTPProxies := make(map[string][]*sesame_api_v1.HTTPProxy)
	...
	for fqdn, proxies := range fqdnHTTPProxies {
		...
		default:
			...
			for _, proxy := range proxies {
				pa, commit := p.dag.StatusCache.ProxyAccessor(proxy)
				pa.Vhost = fqdn
				pa.ConditionFor(status.ValidCondition).AddError(sesame_api_v1.ConditionTypeVirtualHostError,
					"DuplicateVhost",
					msg)
				commit()
			}
...
func getProtocol(service sesame_api_v1.Service, s *Service) (string, error) {
	// Determine the protocol to use to speak to this Cluster.
	var protocol string
...
func toCORSPolicy(policy *sesame_api_v1.CORSPolicy) (*CORSPolicy, error) {
	if policy == nil {
		return nil, nil
	}
...
func toStringSlice(hvs []sesame_api_v1.CORSHeaderValue) []string {
...
func includeMatchConditionsIdentical(includes []sesame_api_v1.Include) bool {
	j := 0
...
func redirectRoutePolicy(redirect *sesame_api_v1.HTTPRequestRedirectPolicy) *Redirect {
	if redirect == nil {
		return nil
	}
