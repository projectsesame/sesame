...
func TestParseTimeoutPolicy(t *testing.T) {
	testCases := map[string]struct {
		config   *sesame_api_v1alpha1.TimeoutParameters
		expected sesameconfig.Timeouts
		errorMsg string
	}{
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			parsed, err := sesameconfig.ParseTimeoutPolicy(tc.config)
