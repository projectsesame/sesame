		...
		"Set to 1": {
			ingress: &networking_v1.Ingress{
				ObjectMeta: metav1.ObjectMeta{
					Name: "ing",
					Annotations: map[string]string{
						"projectsesame.io/num-retries": "1",
					},
				},
			},
			want: 1,
		...
		"Set to 0": {
			ingress: &networking_v1.Ingress{
				ObjectMeta: metav1.ObjectMeta{
					Name: "ing",
					Annotations: map[string]string{
						"projectsesame.io/num-retries": "0",
					},
				},
			},
			want: 1,
		...
		"Set to 9": {
			ingress: &networking_v1.Ingress{
				ObjectMeta: metav1.ObjectMeta{
					Name: "ing",
					Annotations: map[string]string{
						"projectsesame.io/num-retries": "9",
					},
				},
			},
			want: 9,
...
func TestParseUpstreamProtocols(t *testing.T) {
	tests := map[string]struct {
		...
		"empty": {
			a:    map[string]string{"projectsesame.io/upstream-protocol.h2": ""},
			want: map[string]string{},
		},
		"empty with spaces": {
			a:    map[string]string{"projectsesame.io/upstream-protocol.h2": ", ,"},
		...
		"single value": {
			a: map[string]string{"projectsesame.io/upstream-protocol.h2": "80"},
			want: map[string]string{
				"80": "h2",
			},
		},
		"tls": {
			a: map[string]string{"projectsesame.io/upstream-protocol.tls": "https,80"},
			want: map[string]string{
				"80":    "tls",
				"https": "tls",
			},
		...
		"multiple value": {
			a: map[string]string{"projectsesame.io/upstream-protocol.h2": "80,http,443,https"},
			want: map[string]string{
				"80":    "h2",
				"http":  "h2",
				"443":   "h2",
	...
	tests := map[string]struct {
		...
		"single value": {
			a: &networking_v1.Ingress{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						"projectsesame.io/websocket-routes": "/ws1",
					},
				},
			},
			want: map[string]bool{
...
func TestAnnotationCompat(t *testing.T) {
	tests := map[string]struct {
		...
		"projectsesame.io/annotation": {
			value: "200",
			svc: &v1.Service{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						"projectsesame.io/annotation": "200",
					},
				},
			},
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			got := SesameAnnotation(tc.svc, "annotation")
			want := tc.value
			if got != want {
				t.Fatalf("got: %v, want: %v", got, want)
			}
...
func TestAnnotationKindValidation(t *testing.T) {
	...
	for _, kind := range []string{
		...
		kindOf(&sesame_api_v1.HTTPProxy{}),
	} {
