...
func testHTTPSFallbackCertificate(namespace string) {
	Specify("requests with invalid SNI are accepted when a fallback cert is configured", func() {
		...
		f.Certs.CreateSelfSignedCert(namespace, "echo-cert", "echo", "fallback-cert-echo.projectsesame.io")
		...
		p := &sesamev1.HTTPProxy{
			...
			Spec: sesamev1.HTTPProxySpec{
				VirtualHost: &sesamev1.VirtualHost{
					Fqdn: "fallback-cert-echo.projectsesame.io",
					TLS: &sesamev1.TLS{
						SecretName:                "echo",
		...
		res, ok = f.HTTP.SecureRequestUntil(&e2e.HTTPSRequestOpts{
			...
			TLSConfigOpts: []func(*tls.Config){
				e2e.OptSetSNI("invalid-sni-should-use-fallback.projectsesame.io"),
			},
			Condition: e2e.HasStatusCode(200),
		})
		require.NotNil(t, res, "request never succeeded")
