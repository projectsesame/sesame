...
func testEnvoyMetricsOverHTTPS() {
	Specify("requests to metrics listener are served", func() {
		...
		kubectlCmd, err := f.Kubectl.StartKubectlPortForward(18003, 8003, "projectsesame", "daemonset/envoy")
		require.NoError(t, err)
		defer f.Kubectl.StopKubectlPortForward(kubectlCmd)

		clientCert, caBundle := f.Certs.GetTLSCertificate("projectsesame", "metrics-client")
		client := http.Client{
